...
# def read_one_coqa_example_extern():
def read_one_coqa_example_extern(partial_example, QA_history, history_len=2, add_QA_tag=False):
    long_questions=[]
    i = len(QA_history)
    i = 0
...
def convert_one_example_to_features(example, unique_id, example_index, tokenizer, max_seq_length,
...
def read_coqa_examples_dialog(input_file, history_len=2, add_QA_tag=False):
    """Read a CoQA json file into a list of CoqaExample."""
    """Useful Function"""
                ...
                if len(answer) == len(datum['answers']):
                    for ex in answer:
                        ...
                        if idx not in additional_answers:
                            additional_answers[idx] = []
                        additional_answers[idx].append(ex['input_text'])
        for additional_answer_idx in range(1,len(datum['questions'])+1):
            # pdb.set_trace()
            gold_answers_list.append(additional_answers[additional_answer_idx])
        for i in range(len(datum['questions'])):
            question, answer = datum['questions'][i], datum['answers'][i]
            assert question['turn_id'] == answer['turn_id']
            ...
            # _datum['qas'].append(_qas)
            example = CoqaExample(
                # hack xbli replace ' ' with '#' for post process
                qas_id=_datum['id'] + '#' + str(_qas['turn_id']),
