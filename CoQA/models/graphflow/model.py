...
class Model(metaclass=abc.ABCMeta):
    ...
    def __init__(self, config, train_set=None):
        ...
        if self.config['pretrained'] and state_dict_opt:
            # print([self.optimizer.param_groups[i]==state_dict_opt['param_groups'][i] for i in range(len(state_dict_opt['param_groups']))])
            # pdb.set_trace()
...
class CoQAModel(Model):
    ...
    def predict(self, ex, step, update=False, out_predictions=False):
        # Train/Eval mode
        ...
        if (not update) or self.config['predict_train']:
            predictions, spans,tokens,token_spans = self.extract_predictions(ex, score_s, score_e, score_c, res['turn_mask'])
        ...
        for i, (_s, _e, _c) in enumerate(zip(score_s, score_e, score_c)): # Example-level
            ...
            for j in range(_s.size(0)): # Turn-level
                ...
                try:
                    ...
                    para_token_span.append(span)
            predictions.append(para_pred)
            spans.append(para_span)
            para_tokens.append(para_token)
            para_token_spans.append(para_token_span)
        return predictions, spans,para_tokens,para_token_spans
